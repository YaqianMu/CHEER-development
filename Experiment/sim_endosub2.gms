...
set sce instruments /BAU, ECF, LST, ETS/  ;
set target targeted sector /nuclear,wind,solar,Hydro,biomass/  ;
set mapsce(sub_elec,target)  /
nuclear        .        nuclear
wind        .        wind
solar        .        solar
Hydro        .        Hydro
...
tax_s(sub_elec)$mapsce(sub_elec,target)=1-sce_s(sce,target);

ret0(sub_elec) $mapsce(sub_elec,target)=1+(outputelec0(sub_elec)+1*rate(z))/outputelec0(sub_elec);
